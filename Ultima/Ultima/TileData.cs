...
namespace Ultima
{
	...
	public struct LandData
	{
		...
		private short m_TextureID;

		public LandData( string name, TileFlag flags, short textureID )
		{
			m_Name = name;
			m_Flags = flags;
		...
		/// <summary>
		/// Gets the ID of the texture assigned to this land tile, 0 if it doesn't have a texture.
		/// </summary>
			...
			if ( filePath != null )
			{
				using ( FileStream fs = new FileStream( filePath, FileMode.Open, FileAccess.Read, FileShare.Read ) )
				{
					...
					for ( int i = 0; i < 0x4000; ++i )
					{
						...
						short textureID = bin.ReadInt16(); // skip 2 bytes -- textureID

						m_LandData[i] = new LandData( ReadNameString( bin ), flags, textureID );
					}
